...
interface BangalorePlannerCTAProps {
  sourcePage?: string;
  plotL?: number;
  plotW?: number;
}

export function BangalorePlannerCTA({ sourcePage = "bangalore-hub", plotL, plotW }: BangalorePlannerCTAProps = {}) {
  const router = useRouter();

  function buildHref(type: string) {
    const p = new URLSearchParams({ city: "bangalore", type, from: sourcePage });
  ...
  return (
    ...
    >
      <div className="mb-8">
        ...
        >
          What type of home are you building in Bangalore?
        </h2>
        <p
          className="font-sans"
          style={{ fontSize: "18px", lineHeight: 1.7, color: "var(--text-secondary)" }}
        >
          Select your home type to get a Bangalore-specific cost estimate in under 2 minutes.
          Bangalore rates pre-loaded. No city selection needed.
        </p>
      </div>
      ...
      <p className="font-mono mt-5" style={{ fontSize: "13px", color: "var(--text-tertiary)" }}>
        Click a type to start · Bangalore rates pre-loaded · Takes under 2 minutes
      </p>
    </div>
  );
}
