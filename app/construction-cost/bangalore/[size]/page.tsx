...
import { BANGALORE_PLOT_PAGES, BANGALORE_RATES } from "@/lib/data/bangalore-plot-pages";
...
export async function generateStaticParams() {
  return Object.keys(BANGALORE_PLOT_PAGES).map((size) => ({ size }));
}
...
export async function generateMetadata({
  ...
  const config = BANGALORE_PLOT_PAGES[params.size];
  if (!config) return {};
  const pageUrl = `${SITE_URL}/construction-cost/bangalore/${config.slug}`;
  return {
    title: config.title,
...
const RATE_TIER_META = [
  {
    key: "basic" as const,
    label: "Basic",
    badge: null,
    description: "Functional construction. Standard blocks, basic vitrified tiles, no branded items.",
  },
  {
  ...
  },
  {
    ...
    label: "Premium",
    badge: null,
    description: "Stone or large-format flooring, false ceilings, system windows, upgraded sanitaryware.",
  },
  {
    key: "luxury" as const,
    label: "Luxury",
    badge: null,
    description: "Imported marble, hardwood floors, home automation provisions, bespoke architectural finishes.",
  },
];

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function BangalorePlotSizePage({ params }: { params: { size: string } }) {
  const config = BANGALORE_PLOT_PAGES[params.size];
  if (!config) notFound();

  const partner = SEED_PARTNERS.find((p) => p.id === "design-intend")!;
  const pageUrl = `${SITE_URL}/construction-cost/bangalore/${config.slug}`;
  const sourcePage = `bangalore-plot-${config.slug}`;
  ...
  const schemas = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 4, name: `${config.display} House Cost`, item: pageUrl },
      ],
    },
    {
      "@context": "https://schema.org",
    ...
    },
    {
      ...
      operatingSystem: "Web",
      description: "Free construction cost estimator for Bangalore homeowners. Pre-filled for your plot size.",
    },
    {
      "@context": "https://schema.org",
      "@type": "Person",
  ...
  return (
    <>
      ...
      <SiteHeader
        ...
        ctaHref={`/plan?city=bangalore&from=${sourcePage}`}
        maxWidth="max-w-5xl"
      />
        ...
        <section className="max-w-5xl mx-auto px-5 md:px-10 pt-14 pb-10 md:pt-20 md:pb-14">
          <AnimateIn direction="up">
            <nav className="flex items-center gap-2 mb-8 flex-wrap" aria-label="Breadcrumb">
              ...
              <span className="font-mono text-[11px]" style={{ color: "var(--border)" }}>/</span>
              <span className="font-mono text-[11px] uppercase tracking-[0.14em]" style={{ color: "var(--text-primary)" }}>{config.display}</span>
            </nav>

            <p className="font-mono text-[11px] uppercase tracking-[0.2em] mb-2" style={{ color: "var(--accent)" }}>{config.eyebrow}</p>
            ...
            >
              ...
              <p className="font-sans" style={{ fontSize: "clamp(15px, 2vw, 17px)", lineHeight: 1.8, color: "var(--text-primary)" }}>
                Building a {config.display} house in Bangalore in 2026 costs between{" "}
                ...
                Bangalore standard rate:{" "}
                <strong>{fmtRate(BANGALORE_RATES.standard.min)}–{fmtRate(BANGALORE_RATES.standard.max)} per sqft</strong>.
        ...
        <section className="max-w-5xl mx-auto px-5 md:px-10 py-14 md:py-20" aria-labelledby="rate-bands-heading">
          <AnimateIn direction="up">
            <p className="font-mono text-[11px] uppercase tracking-[0.2em] mb-3" style={{ color: "var(--accent)" }}>
              {config.display} plot · Bangalore · {config.floorLabel} · {config.bua.toLocaleString("en-IN")} sqft BUA
            </p>
            <h2
            ...
            <p className="font-sans mb-10" style={{ fontSize: "18px", color: "var(--text-secondary)", maxWidth: "560px", lineHeight: 1.65 }}>
              Ranges computed from verified Bangalore rates × {config.bua.toLocaleString("en-IN")} sqft built-up area.
              Civil construction only. Interiors are billed separately.
            </p>
          </AnimateIn>

          <StaggerContainer className="divide-y divide-border border-t border-border">
            {RATE_TIER_META.map(({ key, label, badge, description }) => {
              const rates = BANGALORE_RATES[key];
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[220px_1fr_200px] gap-4 md:gap-8 py-7 items-start">
                    <div>
                      ...
                      <p className="font-mono" style={{ fontSize: "13px", color: "var(--text-tertiary)" }}>
                        {fmtRate(rates.min)}–{fmtRate(rates.max)}/sqft × {config.bua.toLocaleString("en-IN")} sqft
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 md:justify-end">
                      <div className="w-full md:w-32 h-1 rounded-full overflow-hidden" style={{ background: "var(--border)" }}>
                        <div className="h-full rounded-full" style={{ width: `${Math.round((rates.mid / 5000) * 100)}%`, background: "var(--accent)" }} />
                      </div>
                    </div>
                  </div>
                </StaggerItem>
            ...
            <StaggerItem>
              <div className="grid grid-cols-1 md:grid-cols-[220px_1fr_200px] gap-4 md:gap-8 py-7 items-start">
                ...
                <div className="md:pt-1">
                  ...
                  <Link
                    href={`/plan?city=bangalore&from=${sourcePage}-ultra`}
          ...
          <AnimateIn direction="up" delay={0.1}>
            <p className="font-mono mt-8 pt-6 border-t border-border" style={{ fontSize: "14px", color: "var(--text-tertiary)", lineHeight: 1.7 }}>
              ...
              Full source:{" "}
              <Link href="/construction-cost/bangalore/cost-index" className="underline" style={{ color: "var(--accent)" }}>
                Bangalore Construction Cost Index
              </Link>.
            </p>
          </AnimateIn>
        </section>
          ...
          <AnimateIn direction="up">
            <div className="rounded-sm p-7 md:p-10" style={{ border: "1px solid var(--border)", background: "var(--bg-primary)" }}>
              ...
              <h2 className="font-serif mb-3" style={{ fontSize: "clamp(22px, 3.5vw, 34px)", fontWeight: 400, letterSpacing: "-0.02em", color: "var(--text-primary)" }}>
                Get a personalised {config.display} estimate for Bangalore
              </h2>
              <p className="font-sans mb-6" style={{ fontSize: "18px", color: "var(--text-secondary)", lineHeight: 1.7, maxWidth: "520px" }}>
                Your plot size is pre-filled. Five questions and you get a cost range, material list,
                and payment timeline, built from Bangalore BOQs. No sign-up.
              </p>
              <ul className="mb-8 space-y-2">
                {["Construction cost range by quality tier", "Cost per sqft at your spec", "BOQ summary", "Payment timeline", "Comparison with similar Bangalore builds"].map((item) => (
              ...
              <Link
                href={`/plan?city=bangalore&type=contemporary&length=${config.L}&width=${config.W}&from=${sourcePage}`}
        ...
        <section className="max-w-5xl mx-auto px-5 md:px-10 py-14 md:py-20" aria-labelledby="content-heading">
          <AnimateIn direction="up">
            <h2
              ...
              style={{ fontSize: "clamp(26px, 4vw, 40px)", fontWeight: 400, letterSpacing: "-0.02em", color: "var(--text-primary)" }}
            >
              What to know about a {config.display} plot in Bangalore
            </h2>
            <div className="space-y-6" style={{ maxWidth: "680px" }}>
              {config.body.map((para, i) => (
                <p key={i} className="font-sans" style={{ fontSize: "18px", lineHeight: 1.8, color: "var(--text-secondary)" }}>{para}</p>
              ))}
              <p className="font-mono" style={{ fontSize: "13px", color: "var(--text-tertiary)" }}>
                As of June 2026. BBMP jurisdiction. Rates verified against current Bangalore BOQs.
              </p>
            </div>
          </AnimateIn>
        </section>
        ...
        <section className="max-w-5xl mx-auto px-5 md:px-10 py-14 md:py-20" aria-labelledby="faq-heading">
          <AnimateIn direction="up">
            ...
            <h2
              ...
              style={{ fontSize: "clamp(26px, 4vw, 40px)", fontWeight: 400, letterSpacing: "-0.02em", color: "var(--text-primary)" }}
            >
              {config.display} plot in Bangalore
            </h2>
          </AnimateIn>
          <FAQBlock items={faqItems} schemaId={`bangalore-plot-${config.slug}-faq`} />
        </section>
            ...
            <div className="rounded-sm p-7 md:p-10 grid grid-cols-1 md:grid-cols-[1fr_220px] gap-8 items-start" style={{ border: "1px solid var(--border)", background: "var(--bg-primary)" }}>
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <a href={`mailto:${partner.email}`} className="inline-flex items-center gap-2 px-5 py-2.5 font-mono text-[11px] uppercase tracking-[0.12em] border" style={{ border: "1px solid var(--border)", color: "var(--text-secondary)", borderRadius: "2px" }}>Email</a>
                  {partner.websiteUrl && (
                    <a href={partner.websiteUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 font-mono text-[11px] uppercase tracking-[0.12em] border" style={{ border: "1px solid var(--border)", color: "var(--text-secondary)", borderRadius: "2px" }}>Website ↗</a>
                  )}
                </div>
              </div>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {config.internalLinks.map((link) => (
              <StaggerItem key={link.href}>
                <Link href={link.href} className="block rounded-sm p-6 border transition-colors duration-200 group hover:border-navy" style={{ border: "1px solid var(--border)" }}>
                  <p className="font-mono text-[10px] uppercase tracking-[0.16em] mb-2" style={{ color: "var(--accent)" }}>{link.eyebrow}</p>
                  <p className="font-serif mb-2 group-hover:text-navy transition-colors" style={{ fontSize: "20px", fontWeight: 400, letterSpacing: "-0.01em", color: "var(--text-primary)" }}>{link.title}</p>
                  <p className="font-sans" style={{ fontSize: "15px", color: "var(--text-secondary)", lineHeight: 1.6 }}>{link.desc}</p>
                </Link>
              </StaggerItem>
            ))}
          </StaggerContainer>
          ...
          <div className="max-w-5xl mx-auto px-5 md:px-10 py-14 md:py-20 text-center">
            <AnimateIn direction="up">
              ...
              <h2 className="font-serif mb-4" style={{ fontSize: "clamp(28px, 4.5vw, 48px)", fontWeight: 400, letterSpacing: "-0.025em", lineHeight: 1.1, color: "#FFFFFF" }}>
                Get your {config.display} Bangalore estimate
              </h2>
              <p className="font-sans mb-8 mx-auto" style={{ fontSize: "18px", lineHeight: 1.7, color: "rgba(255,255,255,0.65)", maxWidth: "480px" }}>
                ...
                cost range, material list, and payment timeline, built from Bangalore BOQs.
              </p>
              <Link
                href={`/plan?city=bangalore&type=contemporary&length=${config.L}&width=${config.W}&from=${sourcePage}-footer`}
